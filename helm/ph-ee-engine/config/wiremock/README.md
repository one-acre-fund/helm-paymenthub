...
real sandbox. It's shared infrastructure, not tied to any one connector — today
`ph-ee-connector-mpesa` (stubbing Safaricom's Daraja API) and `ph-ee-connector-mtn`
(stubbing MTN's MoMo Collection API) use it, and further connectors are meant to adopt the
same instance rather than standing up their own.
...
1. Create a `<connector>/mappings/` directory here (mirroring `mpesa/mappings/` or
   `mtn/mappings/`).
2. Name each mapping file `<connector>-NN-description.json` (e.g.
   `airtel-01-oauth-token.json`). The connector prefix is required, not just convention: all
...
4. Add a `<connector>/README.md` documenting what's stubbed, following `mpesa/README.md`
   or `mtn/README.md` as an example.
